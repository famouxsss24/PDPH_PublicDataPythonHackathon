...
실행에 필요한 `data/processed` 산출물(건물·그림자·보행 그래프·장소 색인)은 저장소에 함께 포함된다. 원천 공공데이터, PostGIS, Kakao 키 없이도 위 기본 명령으로 live 장소 검색·경로 비교·3D 화면을 실행한다. `web/mock`은 정적 서버 또는 오프라인 발표용 스냅샷이다.
